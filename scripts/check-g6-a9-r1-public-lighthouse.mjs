...
if (!publicOrigin) throw new Error('MKD_DISTILL_PUBLIC_ORIGIN is required; G6-A9-R1 Lighthouse never falls back to a local target')
if (publicOrigin !== expectedOrigin) throw new Error(`G6-A9-R1 Lighthouse target must be exactly ${expectedOrigin}`)

const expectedRelease = 'mkd-m1c-438e6ae583fc952d'
if (process.env.MKD_DISTILL_EXPECTED_RELEASE !== expectedRelease) throw new Error(`G6-A9-R1 Lighthouse release must be exactly ${expectedRelease}`)
...
const expectedEvidenceRoot = 'output/playwright/mkd-distill-g6a9-r1'
if (evidenceRoot !== expectedEvidenceRoot) throw new Error(`G6-A9-R1 Lighthouse evidence root must be exactly ${expectedEvidenceRoot}`)
...
async function main() {
  ...
  try {
    ...
    const receipt = {
      schemaVersion: 1,
      receiptId: 'MKD-DISTILL-G6A9R1-PUBLIC-LIGHTHOUSE',
