...
if (!publicOrigin) throw new Error('MKD_DISTILL_PUBLIC_ORIGIN is required; public Lighthouse never falls back to a local target')
if (publicOrigin !== expectedOrigin) throw new Error(`public Lighthouse target must be exactly ${expectedOrigin}`)
  ...
  try {
    ...
    for (const target of targets) {
      ...
      for (let run = 1; run <= runsPerTarget; run += 1) {
        ...
        await fs.writeFile(path.join(outputRoot, `${target.name}-run-${run}.report.json`), JSON.stringify(result.lhr, null, 2))
      }
    ...
    const receipt = {
      schemaVersion: 1,
      receiptId: 'MKD-DISTILL-PUBLIC-LIGHTHOUSE',
