...
docId: "KS-PROMPTS"
displayNumber: "15"
route: "/knowledge/15-codex-prompts"
learningOrder: 18
title: "第十五章：Codex Prompts 速查与工具生态"
...
> **阅读方式**：先按阶段选，再看优先级。`[首选]` 表示默认优先尝试，`[推荐]` 表示成熟备选，`[参考]` 表示了解即可或特定场景使用。状态灯含义：`[OK]` 适合当前主流生产实践，`[P1]` 适合试点或特定约束，`[P0]` 仅在遗留系统或强制条件下考虑。

### 采集

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | Firecrawl | [OK] | 网站抓取与清洗 | 动态站点和批量抓取都省心 |
| [首选] | Playwright | [OK] | 浏览器级采集 | 页面复杂、要登录、要点击时必备 |
| [推荐] | Jina Reader | [P1] | 快速网页转 Markdown | 适合轻量公开网页，不适合复杂交互 |

### 提取

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | MinerU | [OK] | PDF/Office 高精度解析 | 混排文档主力 |
| [推荐] | Docling | [OK] | CPU 友好文档解析 | 企业内网部署友好 |
| [参考] | Marker | [P1] | 批量 PDF 解析 | 吞吐高，但要注意协议边界 |

### 结构化提取

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | Ollama + JSON Schema | [OK] | 本地结构化抽取 | 内部数据默认路线 |
| [推荐] | Instructor | [OK] | 结构化输出约束 | 让字段更稳、更少脏数据 |
| [推荐] | Pydantic | [OK] | 字段验证 | 作为 LLM 输出后的硬校验层 |

### 约束输出

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | JSON Schema | [OK] | 约束固定字段输出 | 最通用、最便于审计 |
| [首选] | Pydantic | [OK] | Python 侧校验 | 和 FastAPI、ETL 配合最好 |
| [推荐] | Guardrails | [P1] | 增强约束与重试 | 适合高可靠抽取链路 |

### Embedding

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | BAAI/bge-m3 | [OK] | 多语种通用 Embedding | 兼顾中英和检索质量 |
| [推荐] | jina-embeddings-v3 | [OK] | 高质量检索向量 | 排名稳，适合对比实验 |
| [参考] | e5-large-v2 | [P1] | 经典文本向量 | 老牌可靠，但新项目不必优先 |

### 向量库

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | ChromaDB | [OK] | 本地原型与轻量生产 | 上手极快，适合知识库 PoC |
| [推荐] | Qdrant | [OK] | 生产级向量检索 | 过滤、扩展性都更强 |
| [参考] | FAISS | [P1] | 本地离线索引 | 适合嵌入式或实验场景 |

### 图谱

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | NetworkX | [OK] | 轻量关系图 | 先把关系结构跑通 |
| [推荐] | Neo4j | [OK] | 生产级知识图谱 | 查询强，但维护成本更高 |
| [参考] | ArangoDB | [P1] | 文档+图混合存储 | 适合统一存储诉求 |

### RAG框架

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | LlamaIndex | [OK] | 检索编排与数据连接 | 接入快，组件多 |
| [首选] | LangChain | [OK] | RAG 基础框架 | 生态大，但注意控制复杂度 |
| [推荐] | Haystack | [P1] | 检索评估与管线化 | 企业搜索场景更常见 |

### Agent框架

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | FastMCP | [OK] | Tool 暴露与 Agent 接入 | 做“可调用能力”最直接 |
| [推荐] | PydanticAI | [OK] | 类型安全 Agent | 适合强调结构化输出 |
| [参考] | AutoGen | [P1] | 多 Agent 协同 | 场景复杂时再上 |

### MCP协议

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | FastMCP | [OK] | Python MCP Server | 目前最顺手 |
| [推荐] | MCP Python SDK | [OK] | 标准协议实现 | 追求规范化时使用 |
| [参考] | MCP TypeScript SDK | [P1] | Node 侧 MCP 开发 | 前端或 JS 栈团队更合适 |

### 工作流编排

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | APScheduler | [OK] | 轻量定时任务 | 每日扫描、报表生成够用 |
| [推荐] | Airflow | [OK] | 批量 ETL 编排 | DAG 清晰，适合多步骤生产流 |
| [参考] | Prefect | [P1] | Python 友好编排 | 开发体验不错，团队需额外熟悉 |

### 本地LLM

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | Ollama | [OK] | 本地模型统一入口 | 内部数据默认标准答案 |
| [推荐] | vLLM | [OK] | 高吞吐服务化推理 | 并发高时更划算 |
| [参考] | LM Studio | [P1] | 桌面实验推理 | 个人试验方便，不是生产主力 |

### 评估

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | Pytest | [OK] | 检索与 API 回归测试 | 最容易纳入 CI |
| [首选] | Ragas | [OK] | RAG 质量评估 | 适合快速量化召回与回答 |
| [推荐] | DeepEval | [P1] | LLM 评估框架 | 适合做更丰富的实验 |

### 监控

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | Prometheus | [OK] | 指标采集 | API、任务、延迟都能统一监控 |
| [推荐] | Grafana | [OK] | 可视化看板 | 和 Prometheus 配套最稳 |
| [推荐] | Streamlit | [P1] | 业务型监控面板 | 做成本和评估报表很快 |

### 安全

| 优先级 | 工具 | 状态 | 核心用途 | 一句话备注 |
| --- | --- | --- | --- | --- |
| [首选] | Presidio | [OK] | PII 检测与脱敏 | 内部数据入库前先过这一层 |
| [推荐] | Vault | [OK] | 密钥管理 | 不要把凭证塞进代码和 .env 提交 |
| [推荐] | Trivy | [P1] | 镜像与依赖扫描 | 上线前做基础供应链检查 |
...
## 15.7 Prompt 元数据规范：版本、适用模型、过期信号

Prompt 不是永久有效的资产，它是**当前模型能力与任务需求之间的临时协议**。没有元数据的 Prompt 库，六个月后会变成技术债仓库。

### 每个 Prompt 必须标注的五项元数据
...
prompt_metadata:
  ...
  tested_models:
    - model: "claude-opus-4-7"
      performance: "A"         # A/B/C/D
      tested_date: "2026-07-01"
    - model: "gpt-4o"
      performance: "B"
  ...
  # 过期信号：出现这些情况时需要重新评估
  expiry_signals:
    - "目标模型发布新版本且有breaking change"
    - "任务成功率连续两周低于85%"
    - "业务场景定义发生变更"
    - "输入数据格式有重大调整"
  
  # 替换策略
  replacement_strategy: "先在沙箱测试新版本，通过黄金样本集后才替换"
  
  # 负责人（谁在Prompt失效时第一个被通知）
  owner: "工程负责人"
...
| 变化类型 | 影响范围 | 检测方式 |
|----------|----------|----------|
| **模型升级** | 输出格式/推理方式可能改变 | 模型发布公告 + 定期回归测试 |
| **任务定义漂移** | 用户期望与 Prompt 设计不再一致 | 用户反馈 + 任务成功率监控 |
| **输入数据变化** | Prompt 假设的数据结构已不存在 | Schema 变更日志 |

:::tip 最小可行版本
没有时间管理所有 Prompt 的元数据？至少给**高频使用（每天 > 50 次）** 和 **高风险（L3 以上场景）** 的 Prompt 加上 `owner` 和 `expiry_signals` 两个字段。这两个字段能确保 Prompt 失效时有人知道。
:::
...
提示词是个人工作流速查，不构成 Agent 产品合同；不证明这些 prompt 在 Reference 黄金任务上有效。
