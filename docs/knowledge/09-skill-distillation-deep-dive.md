...
| **[!] 核心缺陷** | — | **仅适用于结构清晰的书籍/文章**；对口语化的视频、对话内容蒸馏效果下降明显 |
...
| **[!] 核心缺陷** | — | **幻觉风险极高**：Agent 在模拟已知人格时，会用自己的知识"补充"，产生从未存在过的"引用" |
...
| **[!] 核心缺陷** | — | **Human-in-the-Loop 负担**：关键节点需要人工审核，对小团队有额外时间成本 |
...
| 评估维度 | 路线一：方法论（cangjie）| 路线二：人格克隆（nuwa）| 路线三：工程操作（R2S）| 路线四：Skill进化（darwin）|
| :--- | :---: | :---: | :---: | :---: |
| **Agent 任务完成率** | ★★★★★ | ★★★☆☆ | ★★★★★ | N/A |
| **信息保真度** | ★★★★☆ | ★★★☆☆ | ★★★★★ | ★★★★★ |
| **幻觉风险** | 低 | **高** | 低 | 极低 |
...
步骤：
1. cangjie 从源材料生成 SKILL.md v1（方法论骨架）
2. 上线运行 2-4 周，收集成功/失败案例
3. darwin 基于案例做棘轮迭代（注意：必须配合评分器校准）
4. 重复步骤 2-3，直到 9 维度评分稳定
...
步骤：
1. R2S 从多模态资源（教程视频+代码+文章）提炼可执行操作步骤
   输出：带执行步骤的 SKILL.md 骨架（知道"做什么"）
2. cangjie 从相关书籍/专家文章提炼判断逻辑和例外处理
   输出：补充"为什么这样做"和"什么情况不适用"
3. 人工合并两份产出，确保风格一致
...
步骤：
1. nuwa 从公开资料提炼目标人物的表达风格和思维模式
   输出：风格层 SKILL.md（知道"怎么说"）
2. cangjie 从其著作提炼核心方法论
   输出：方法论层 SKILL.md（知道"说什么"）
3. 组合时保持方法论内容不变，只用风格层调整表达方式
...
步骤：
1. 收集 Skill 的真实使用记录（成功案例+失败案例各 ≥ 20 条）
2. darwin 生成改进建议
3. 人工审核改进建议是否符合原始设计意图
4. 通过版本回归测试后才允许合并

禁止事项：
- 不要在没有失败案例的情况下跑 darwin（它会优化无意义的细节）
- 不要让 darwin 单次修改超过 20% 的内容（单步变化过大难以归因）
```

### 混编的禁忌

:::warning 三条混编禁忌
1. **不要并行合并**：多路线应顺序执行，不要把两份产出直接 merge——血缘不清晰，出问题无法归因
