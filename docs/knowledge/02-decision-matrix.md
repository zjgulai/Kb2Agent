...
| 步骤 | 核心问题 | 判断结果 | 若跳过此步的后果 |
|------|---------|---------|----------------|
| **Step 1 半衰期** | 这条数据3个月后还有用吗？更新频率是什么？ | <48h → 缓存层；1周-1年 → 入库；>1年 → 深蒸馏 | 把实时数据入了向量库，Agent拿过期事实作答 |
| **Step 2 产品形态** | 谁用这个系统？怎么接入？ | A-E 五种形态（见§2.0） | 架构与用户期望不匹配，做完才发现要推倒重来 |
| **Step 3 输入类型** | 数据来自哪里？格式是什么？ | A-J 十种场景（见§2.1）| 用错SOP，丢失大量有效信息 |
| **Step 4 输出形式** | 知识最终以什么形式存储和被调用？ | Skill/Wiki/规则/模型训练集（见§2.2-2.3）| 蒸馏深度错误，要么白蒸馏要么用不了 |
| **Step 5 混合例外** | 输入是否跨多个类型？能否直接归类？ | Triage分诊（见§2.6）| 强行归到单一场景，丢失跨模态信息 |
...
决策矩阵是选型启发式，不能替代对具体语料、许可与任务的编译验证；矩阵结论不自动授权任何实现。
